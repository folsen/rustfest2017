...
impl Rand for Color {
	fn rand<R: Rng>(rng: &mut R) -> Self {
		let colors = [Color::Blue, Color::Green, Color::Orange, Color::Purple, Color::Red, Color::White];
...
fn row_neighbours(cell: &Cell) -> Vec<Cell> {
	...
	if cell.0 < 7 { cells.push((cell.0 + 1, cell.1)); }
	if cell.0 > 0 { cells.push((cell.0 - 1, cell.1)); }
	cells
}
...
fn col_neighbours(cell: &Cell) -> Vec<Cell> {
	let mut cells = Vec::new();
	if cell.1 < 7 { cells.push((cell.0, cell.1 + 1)); }
	if cell.1 > 0 { cells.push((cell.0, cell.1 - 1)); }
	cells
}
...
impl Move {
	...
	pub fn is_valid(&self) -> bool {
		if self.row1 < 8 && self.col1 < 8 && self.row2 < 8 && self.col2 < 8
			&& (self.row1 == self.row2 && cmp::max(self.col1, self.col2) - cmp::min(self.col1, self.col2) == 1
			|| cmp::max(self.row1, self.row2) - cmp::min(self.row1, self.row2) == 1 && self.col1 == self.col2) {
				return true
		}
		false
	}
...
impl Game {
	...
	fn clear_board(&mut self, first_loop: bool) -> i32 {
		...
		for x in xs {
			for i in 0..x.0 {
				self.execute_move(&Move { row1: x.0 - 1 - i, col1: x.1, row2: x.0 - i, col2: x.1 });
			}
	...
	fn contiguous_loop<F>(&self, contiguous_cells: &mut HashSet<Cell>, visited: &mut HashMap<Cell, bool>, cell: Cell, f: &F)
		where F: Fn(&Cell) -> Vec<Cell> {
		...
		for neighbour in f(&cell) {
			if self.grid[cell.0][cell.1] == self.grid[neighbour.0][neighbour.1] && !visited.contains_key(&neighbour){
